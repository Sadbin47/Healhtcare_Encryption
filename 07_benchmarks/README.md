...
```bash
python3 07_benchmarks/benchmark_crypto.py \
  ../../06_Datasets_and_Data/EHR.csv --repeats 10 --warmups 2

python3 07_benchmarks/benchmark_workflow.py \
  ../../06_Datasets_and_Data/EHR.csv --repeats 10 --warmups 2
```

Outputs are written to `07_benchmarks/results/` as JSON summaries and raw CSV
...
VPS HTTPS, Anvil/Hardhat/provider transactions, TLS network latency, and
WireGuard latency.
